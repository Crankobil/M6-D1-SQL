...
UPDATE 
	public.books 
		SET 
		cover='https://www.imdb.com/title/tt0382625/mediaviewer/rm3939601664/?ref_=ext_shr_lnk'
	WHERE id=1;


-- Select books from a category (any)
SELECT * FROM public.books WHERE category='Horror'

-- Select books which start with a letter (any letter)
SELECT * FROM public.books WHERE name LIKE 'S%'

-- Select books which include a given word  (any word)
SELECT * FROM public.books WHERE name LIKE '%Maximum%'
